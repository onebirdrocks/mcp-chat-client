...
export default function EditMCPServerModal({ server, onClose, onSubmit }: EditMCPServerModalProps) {
  ...
  useEffect(() => {
    ...
    setValue('command', server.command);
    setValue('args', server.args ? server.args.join(' ') : '');
    setValue('env', server.env ? JSON.stringify(server.env, null, 2) : '');
  }, [server, setValue]);

  const handleFormSubmit = async (data: EditServerFormData) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4 shadow-xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Edit MCP Server</h2>
          <button
            onClick={handleClose}
